...
class PairTrading:
    def __init__(self, df1, df2, df_beta, df_multiplier, window_list1, window_list2, open_list1, open_list2,
                 gain_list, loss_list, base_amount, start):
    ...
    def combine_backtesting(self):  # this func reports the execution results of the pair_trading_backtesting
        ...
        #
        total_record_col = ["短线window", "短线开仓信号（500-1000收益率）", "长线window",
                        ...
                        for stop_gain in self.gain_list:
                            for stop_loss in self.loss_list:
                                df_combine, df_record, info = \
                                    self.pair_trading_backtesting(window1, window2, open1, open2, stop_gain, stop_loss)
                                df_line = pd.DataFrame(info, index=[0])
                                df_record_final = pd.concat([df_record_final, df_line])
        df_record_final.to_csv("result-const_vol/Combined Final Result with amount of CSI500={} {}.csv".
    ...
    def pair_trading_backtesting(self, window1, window2, open1, open2, stop_gain, stop_loss):
        ...
        for index in range(0, len(date)):
            ...
            df_combine.loc[date[index], 'cd3'] = df_combine.loc[date[index], 'rolling_difference_long'] < \
                                                 - open2 * np.sqrt(df_combine.loc[date[index], 'ht2'])
            df_combine.loc[date[index], 'cd4'] = df_combine.loc[date[index], 'rolling_difference_long'] > \
                                                 open1 * np.sqrt(sig1)
            df_combine.loc[date[index], 'cd5'] = df_combine.loc[date[index], 'rolling_difference_long'] < \
                                                 - open2 * np.sqrt(sig2)

        # df_combine.to_csv('with sig1 and sig2_1.csv')
        df_combine["Status"] = df_combine['cd4'] & df_combine['cd5']
        ...
        while index < len(df_combine) - 1:
            ...
            index1, info = self.cal_strategy(df_combine, index_start, stop_gain, stop_loss)
            print(df_record)
            df_line = pd.DataFrame(info, index=[0])
        ...
        info_final = dict(zip(total_record_col,
                              [window1, open1, window2, open2, stop_gain, stop_loss, number_record, number_gain,
                               pct_gain, duration_mean, duration_gain, duration_loss, total_gain_pct, total_gain,
                               self.base_amount, sharpe]))
        df_combine.to_csv(
            "result-const_vol/Original window1={} open1={} window2={} open2={} gain={}loss={} base={} date={}.csv".
            format(window1, open1, window2, open2, stop_gain, stop_loss, self.base_amount, self.start),
            encoding="gbk")
        df_record.to_csv(
            "result-const_vol/Record window1={} open1={} window2={} open2={} gain={}loss={} base={} date={}.csv".
            format(window1, open1, window2, open2, stop_gain, stop_loss, self.base_amount, self.start),
            encoding="gbk", index=False)
        return df_combine, df_record, info_final

    def cal_strategy(self, df_combine, index_start, stop_gain, stop_loss):
        index = index_start
        ...
        index += 1
        while index < len(df_combine) - 1 and stop_loss < df_combine.loc[
            index_list[index - 1], "PnL_Combine_pct"] < stop_gain:
            df_combine.loc[index_list[index], "PnL_Combine"] = \
                df_combine.loc[index_list[index_start]:index_list[index], "PnL_day"].sum()
            df_combine.loc[index_list[index], "PnL_Combine_pct"] = df_combine.loc[index_list[index], "PnL_Combine"] / \
                                                                   (df_combine.loc[
...
if __name__ == "__main__":
    ...
    window_list1 = [20, 30, 60]  # [10, 20, 60]
    open_list1 = [0.01, 0.015, 0.02]
    # window_list2 = [120, 180, 240, 300]
    window_list2 = [120, 180, 240]  # [180, 240, 300]
    open_list2 = [-0.01, -0.015, -0.02]  # [-1.2, -1.3]
    ...
    PairTrading(df_500, df_1000, df_beta, df_multiplier, window_list1, window_list2, open_list1, open_list2,
                gain_list, loss_list, 10000, "201511")
    # PairTrading(df_500, df_1000, df_beta, df_multiplier, window_list1, window_list2, open_list1, open_list2,
