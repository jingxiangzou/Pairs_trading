...
class PairTrading:
    def __init__(self, df1, df2, df_beta, df_multiplier, window_list1, window_list2, open_list1, open_list2,
                 gain_list, loss_list, base_amount, start, rolling_window, mean_revert):
        ...
        self.open_list1 = open_list1
        # threshold for open
        self.open_list2 = open_list2
        # threshold condition for validity of the strategy when the long term negative return is not too big : -8% -10%
        self.gain_list = gain_list
        self.loss_list = loss_list
        self.base_amount = base_amount  # amount of position
        self.start = start
        self.rolws = rolling_window  # window for calculating the spread volatility, in terms of trading days
        self.mr = mean_revert
        # the threshold which signals the closing of position, one threshold for both short and long
        # one good value for the mean reverting shreshold is 0.001
        self.combine_backtesting()

        # the function in the self init place is going to execute automatically whenever the class is initialized

    def combine_backtesting(self):
        # this func reports the execution results of the pair_trading_backtesting
        # per different values for the parameters
        #
        total_record_col = ["滚动窗", "短线window", "短线开仓信号（500-1000收益率）", "长线window",
                    ...
                    for open2 in self.open_list2:
                        for stop_gain in self.gain_list:
                            for stop_loss in self.loss_list:
                                for rol_wind in self.rolws:
                                    for mrv in self.mr:
                                        df_combine, df_record, info = \
                                            self.pair_trading_backtesting(window1, window2, open1, open2, stop_gain, stop_loss, rol_wind, mrv)
                                        df_line = pd.DataFrame(info, index=[0])
                                        df_record_final = pd.concat([df_record_final, df_line])
        df_record_final.to_csv("result_rolling/Combined Final Result with amount of CSI500={} {}.csv".
    ...
    def pair_trading_backtesting(self, window1, window2, open1, open2, stop_gain, stop_loss, rol_wind, mrv):
        ...
        for index in range(rol_wind, len(date)):
            df_combine.loc[date[index], 'rol_sig1'] = np.std(df_combine.loc[date[index-rol_wind: index], 'rolling_difference_short'])
            df_combine.loc[date[index], 'rol_sig2'] = np.std(df_combine.loc[date[index-rol_wind: index], 'rolling_difference_long'])

        self.mu_short = df_combine['rolling_difference_short'].mean()
        self.mu_long = df_combine['rolling_difference_long'].mean()
        ...
        for index in range(rol_wind, len(date)):
            df_combine.loc[date[index], 'cd1'] = df_combine.loc[date[index], 'rolling_difference_short'] > \
                                                 open1 * np.sqrt(df_combine.loc[date[index], 'rol_sig1'])
            df_combine.loc[date[index], 'cd2'] = df_combine.loc[date[index], 'rolling_difference_long'] > \
                                                 open2 * np.sqrt(df_combine.loc[date[index], 'rol_sig2'])
        ...
        # PnL are of course calculated daily
        # yet the generation of trading signal is based on a window amount
        ...
        if True in status_list:
            ...
            index = index_start
            # here because the condition hereunder is 'index < len(df_combine) thus no exe

        # rv 'index' is set to be the same as index_start, but keep in mind that it is just used in iteration
        # 'index_start' is a constant: the day of open
        ...
        total_record_col = ["回归线", "滚动窗", "短线window", "短线开仓信号（500-1000收益率）", "长线window",
                            "长线filter信号（500-1000收益率）",
                            "止盈线", "止损线", "总开仓次数", "赚钱次数",
        ...
        # avg time of keeping the position
        duration_gain = df_record.loc[df_record["total_pnl_pct"] > 0, "duration"].mean()
        # avg time of keeping the position which is finally profitable
        duration_loss = df_record.loc[df_record["total_pnl_pct"] < 0, "duration"].mean()
        # avg time of keeping the position whih is not profitable
        ...
        info_final = dict(zip(total_record_col,
                              [mrv, rol_wind, window1, open1, window2, open2, stop_gain, stop_loss, number_record,
                               number_gain, pct_gain, duration_mean, duration_gain, duration_loss,
                               total_gain_pct, total_gain, self.base_amount, sharpe]))
        df_combine.to_csv(
            "result_rolling/Original window1={} open1={} window2={} open2={} gain={}loss={} base={} date={} rwd= {} mrv={}.csv".
            format(window1, open1, window2, open2, stop_gain, stop_loss, self.base_amount, self.start, rol_wind, mrv),
            encoding="gbk")
        df_record.to_csv(
            "result_rolling/Record window1={} open1={} window2={} open2={} gain={}loss={} base={} date={} rwd= {} mrv={}.csv".
            format(window1, open1, window2, open2, stop_gain, stop_loss, self.base_amount, self.start, rol_wind, mrv),
            encoding="gbk", index=False)
        return df_combine, df_record, info_final

    def cal_strategy(self, df_combine, index_start, stop_gain, stop_loss, mrv):
        """ set the condition for closing the position"""
        index = index_start
        ...
        while index < len(df_combine) - 1 and stop_loss < df_combine.loc[index_list[index - 1], "PnL_Combine_pct"] \
            ...
            df_combine.loc[index_list[index], "PnL_Combine_pct"] = df_combine.loc[index_list[index], "PnL_Combine"] / \
                                                                   (df_combine.loc[
                                                                        index_list[
                                                                            index_start], "收盘价1"] * self.base_amount)
            # the definition of the combine pct has rendered itself meaningless
            index += 1
...
if __name__ == "__main__":
    ...
    window_list1 = [5, 10, 15,  20, 25, 30, 35, 40, 45, 50, 55, 60]
    open_list1 = [0.03]
    # window_list2 = [120, 180, 240, 300]
    window_list2 = [120]  # [180, 240, 300]
    open_list2 = [-0.05]  # [-1.2, -1.3]
    ...
    PairTrading(df_500, df_1000, df_beta, df_multiplier, window_list1, window_list2, open_list1, open_list2,
                gain_list, loss_list, 10000, "201511", rolling_windows, mean_reverts)
    # PairTrading(df_500, df_1000, df_beta, df_multiplier, window_list1, window_list2, open_list1, open_list2,
